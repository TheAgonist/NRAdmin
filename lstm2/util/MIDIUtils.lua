...
function MIDIUtils.text_to_tensor(in_midifile, out_vocabfile, out_tensorfile)
  ...
  for i=0,#datas do
    ...
    tot_len= tot_len+math.floor((last_note )/(rawdata[1]/32)+1)
    ...
    for i=1, #rawdata[indexer] do
      if rawdata[indexer][i] ~= nil then
        if rawdata[indexer][i][1]=="note" and rawdata[indexer][i][2]~=0 then
            currnote= (rawdata[indexer][i][2] )/(rawdata[1]/32)+1
            ...
            data[(currlen+currnote][rawdata[indexer][i][5]] = vocab_mapping[rawdata[indexer][i][5]+ 127*tempIndex]
            --print(data[currlen+currnote].."   "..vocab_mapping[rawdata[indexer][i][5]+ 127*tempIndex])
            --if data[(currlen+currnote][rawdata[indexer][i][5]]==0 then data[currlen+currnote[rawdata[indexer][i][5]]=data[currlen+currnote]+1
           --print(data[currlen+currnote].."   "..vocab_mapping[rawdata[indexer][i][5]+ 127*tempIndex]) 
           --os.exit()
         end
        end 
